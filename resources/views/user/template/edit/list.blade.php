         ...
         <div class="list-option-area">
            ...
            <div class="list-area">
                     ...
                     <div class="row list-item-area1">
                        ...
                        <div class="col-6 item-{{ $sectionKey+1 }}-{{ $rowKey+1 }}">
                           <div class="form-group">
                              ...
                              @if($rowKey != 0)
                              <a href="javascript:void(0)" class="float-right btn btn-sm btn-danger remove-option-item" data-addbutton=".option-item-btn1" data-target=".item-{{ $sectionKey+1 }}-{{ $rowKey+1 }}">{{ __('X') }}</a>
                              @endif
                              <input  type="text" class="form-control" name="section[1][value][{{$rowKey}}][description]" placeholder="{{ __('Example: Banana is a healthy food') }}" value="{{ $row['description'] ?? '' }}"   maxlength="50" />
                           </div>
                        </div>
                        @endforeach
                     </div>
               ...
               <div class="card card-primary card-item card-area{{$sectionKey+1}}">
                  <div class="card-header">
                     <h4>{{ __('List :expr', ['expr' => $sectionKey+1]) }}</h4>
                  ...
                  <div class="card-body">
                     <div class="row">
                        <div class="col-sm-12">
                           <div class="form-group">
                              <label>{{ __('List Section Title') }}</label>
                              <input type="text" class="form-control" name="section[{{$sectionKey+1}}][title]" placeholder="{{ __('Example: Select a fruit') }}"  required="" maxlength="50"  value="{{ $section['title'] ?? '' }}" />
                           </div>
                        </div>
                     </div>
                     <div class="row list-item-area{{$sectionKey+1}}">
                        ...
                        <div class="col-6 item-{{ $sectionKey+1 }}-{{ $rowKey+1 }}">
                           <div class="form-group">
                              <label>{{ __('Enter List Value Name') }}</label>
                              <input type="text" class="form-control itemval-{{$sectionKey+1}}" name="section[{{$sectionKey+1}}][value][{{$rowKey}}][title]" placeholder="{{ __('Example: Banana') }}" value="{{ $row['title'] ?? '' }}" required="" maxlength="50" />
                           </div>
                        </div>
                        <div class="col-6 item-{{ $sectionKey+1 }}-{{ $rowKey+1 }}">
                           <div class="form-group">
                              ...
                              <a href="javascript:void(0)" class="float-right btn btn-sm btn-danger remove-option-item" data-addbutton=".option-item-btn1" data-target=".item-{{ $sectionKey+1 }}-{{ $rowKey+1 }}">{{ __('X') }}</a>
                              @endif
                              <input type="text" class="form-control" name="section[{{$sectionKey+1}}][value][{{$rowKey}}][description]" placeholder="{{ __('Example: Banana is a healthy food') }}" value="{{ $row['description'] ?? '' }}" maxlength="50" />
                           </div>
                        </div>
                        @endforeach
                     </div>
