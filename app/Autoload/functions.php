...
if (! function_exists('amount_format')) {
    ...
    function amount_format($amount = 0, $icon_type = 'name')
    {
        ...
        if ($icon_type == 'name') {
            $currency = $currency->position == 'right' ? __($currency->name).' '.number_format($amount, 2) : number_format($amount, 2).' '.__($currency->name);
        } elseif ($icon_type == 'both') {
            $currency = __($currency->icon).' '.number_format($amount, 2).' '.__($currency->name);
        } else {
            $currency = $currency->position == 'right' ? number_format($amount, 2).' '.__($currency->icon) : __($currency->icon).' '.number_format($amount, 2);
        }
...
if (! function_exists('get_option')) {
     ...
     * @param $lang
     */
    function get_option($key, bool $decode = false, bool $locale = false, $associative = false, $language = null): mixed
    {
        if ($locale || $language) {
            $tail = $language ?? $locale;
            $cacheKey = $key.$tail;
        } else {
            $cacheKey = $key;
        }
        $cacheKey =
        $option = cache_remember($cacheKey, function () use ($key, $locale, $language) {
            $row = Option::query();
            if ($locale) {
                $row = $row->where('lang', current_locale());
            }
            if ($language) {
                $row = $row->where('lang', $language);
            }

            return $row->where('key', $key)->first();
