...
class Optionupdate
{
    ...
    public function primaryDataUpdate(Request $request)
    {
        ...
        $locale = $request->lang ?? current_locale();
        $primary_data = Option::where('key', 'primary_data')->where('lang', $request->lang)->first();

        if (empty($primary_data)) {

            $primary_data = new Option;
            $primary_data->key = 'primary_data';
            $primary_data->lang = $locale;

            if ($request->hasFile('logo')) {
                $logo = $this->saveFile($request, 'logo');
                $optionData['logo'] = $logo;
            }
    ...
    public function contactPageUpdate(Request $request)
    {
        ...
        $contact = Option::where('key', 'contact-page')->where('lang', $locale)->first();
        if (empty($contact)) {
            $contact = new Option;
